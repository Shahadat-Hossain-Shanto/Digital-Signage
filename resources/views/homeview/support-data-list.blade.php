...
<head>
  ...
  <title>ScreenCast | 24/7 Support</title>
...
<body>
  ...
  <script>
    ...
    $(document).ready(function () {
      $('#supportTable').DataTable({
        responsive: true,
        "ajax": "/support-requests",  // Route to fetch data
        "columns": [
          { "data": "ticketNumber" },
          { "data": "name" },
          { "data": "email" },
          ...
          { "data": "status" },
          {
            ...
            "fnCreatedCell": function (nTd, sData, oData) {
              $(nTd).html(
                `<a style='margin-right: 3px; padding: 3px 7px;'
                    class='btn btn-danger'
                    onclick='deleteEntry(${oData.id});'><i class='fa fa-trash'></i></a>
                <a style='margin-right: 3px; padding: 3px 6px;'
                    class='btn btn-info'
                    href='/edit-support-data-list/${oData.id}'><i class='fa fa-pencil'></i></a>`
              );
            }
          }
        ]
  ...
  if (confirm("Are you sure you want to delete this entry?")) {
    $.ajax({
      url: "/support-request/" + id,  // Ensure this matches your delete route
      type: 'DELETE',  // Use DELETE method for this request
    ...
    function editEntry(id) {
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#issue_description').val(data.issue_description);
          $('#status').val(data.status);
          // Save the ID to use in the update
          $('#supportForm').data('id', id);
        }
      });
    }
